...
void listarObraPorCaixa(int numCaixa, char *opcaoObra) {
    ...
    while (elementoTmp != NULL) {
        if (strcmp(elementoTmp->dados_obra->tipo_obra,opcaoObra) == 0) {
            ...
            printf("CAIXA: %d   ",elementoTmp->dados_obra->caixa+1);
            printf("AUTOR: %s   \n",elementoTmp->dados_obra->autor);
            printf("EDITORA: %s\n",elementoTmp->dados_obra->editora);
            printf("NRO EDICAO: %s  ",elementoTmp->dados_obra->edicao);
            printf("QTD EXEMPLARES: %s\n",elementoTmp->dados_obra->qtdExemplares);
            printf("NRO ISBN: %s    ",elementoTmp->dados_obra->isbn);
...
void menuSistema() {
    ...
    while (opcaoMenu != 5) {
        ...
        printf("DISCIPLINA: ESTRUTURA DA DADOS I                 \n");
        printf("#################################################\n");
        printf("\n");
        printf("SISTEMA DE GERENCIAMENTO DE OBRAS. ESCOLHA A OPCAO DESEJADA:\n");
        ...
        case 2:
            system("clear");
            qtdCaixa = 0;
            while (qtdCaixa < qtdCaixas) {
                listarObraPorCaixa(qtdCaixa,"LIVRO");
                qtdCaixa++;
            }
            pausar();
            system("clear");
        ...
        case 3:
            ...
            qtdCaixa = 0;
            while (qtdCaixa < qtdCaixas) {
                listarObraPorCaixa(qtdCaixa,"REVISTA");
                qtdCaixa++;
            }
            pausar();
            system("clear");
...
// **** BUSCA O ARQUIVO.TXT COM AS OBRAS JA ARMAZENADAS
    ...
    while(fgets(buffer, BUFSIZ, arquivo) != NULL) {
        ...
        if (dados->caixa > c->tamanho)
            c->tamanho = dados->caixa;
        addElementoCaixa(dados);
    }
    if (c->tamanho == 0) {
        c->tamanho = qtdCaixas-1;
    }

}
int get_size(FILE* file_name) {
    file_name = fopen("arquivo.txt","r");
    if(file_name == NULL)
        return 0;
